...
import cn.shanghai.oyb.compileflow.editor.cells.CompileFlowEndCell;
 ...
 * @author ouyubin
 */
public class CompileFlowEndGraphEditPart extends CompileFlowElementEditPart {


    public CompileFlowEndGraphEditPart(Object model, TAdaptable adapter) {
        super(model, adapter);
    }
    ...
    public BaseGeometry createGeometry() {
        ...
        double y = 0.0;
        //-设置初始大小
        double width = CompileFlowGeometryConstants.END_GEOMETRY[0];
        double height = CompileFlowGeometryConstants.END_GEOMETRY[1];
        return new BaseGeometry(x, y, width, height);
    }

    @Override
    public BaseCell createCell(Geometry geometry) {
        return new CompileFlowEndCell(getModel(), geometry);
    }
    ...
    public void addSourceConnection() {
        ...
        if (parentCell instanceof Cell) {
            ...
            this.getMyCell().insertEdge(flowTransitionEdge, true);
        }
    }
                    ...
                    if (object instanceof EdgeElement) {
                        ...
                        for (XmlTag xmlTag : transitionXmlTags) {
                            ...
                            if (StringUtils.equals(toValue, currentName)) {
                                ...
                                this.getMyCell().insertEdge((CompileFlowTransitionEdge) edge, false);
                            }
                        }
                    }
                }
