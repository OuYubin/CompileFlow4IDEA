...
import cn.shanghai.oyb.compileflow.editor.cells.CompileFlowDecisionCell;
 ...
 * @author ouyubin
 */
public class CompileFlowDecisionGraphEditPart extends CompileFlowElementEditPart {

    public CompileFlowDecisionGraphEditPart(Object model, TAdaptable adapter) {
        super(model, adapter);
    }

    @Override
    public BaseCell createCell(Geometry geometry) {
        return new CompileFlowDecisionCell(getModel(), geometry);
    }
    ...
    public BaseGeometry createGeometry() {
        ...
        double y = 0.0;
        //-设置初始大小
        double width = CompileFlowGeometryConstants.DECISION_GEOMETRY[0];
        double height = CompileFlowGeometryConstants.DECISION_GEOMETRY[1];
