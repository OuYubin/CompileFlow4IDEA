...
import cn.shanghai.oyb.compileflow.editor.cells.CompileFlowContinueCell;
 ...
 * @author ouyubin
 */
public class CompileFlowContinueGraphEditPart extends CompileFlowElementEditPart {

    public CompileFlowContinueGraphEditPart(Object model, TAdaptable adapter) {
        super(model, adapter);
    }
    ...
    public BaseGeometry createGeometry() {
        ...
        double y = 0.0;
        //-设置初始大小
        double width = CompileFlowGeometryConstants.CONTINUE_GEOMETRY[0];
        double height = CompileFlowGeometryConstants.CONTINUE_GEOMETRY[1];
        return new BaseGeometry(x, y, width, height);
    }

    @Override
    public BaseCell createCell(Geometry geometry) {
        return new CompileFlowContinueCell(getModel(), geometry);
    }
