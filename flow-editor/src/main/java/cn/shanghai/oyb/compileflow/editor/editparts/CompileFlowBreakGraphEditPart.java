...
import cn.shanghai.oyb.compileflow.editor.cells.CompileFlowBreakCell;
 ...
 * @author ouyubin
 */
public class CompileFlowBreakGraphEditPart extends CompileFlowElementEditPart {


    public CompileFlowBreakGraphEditPart(Object model, TAdaptable adapter) {
        super(model, adapter);
    }
    ...
    public BaseGeometry createGeometry() {
        ...
        double y = 0.0;
        //-设置初始大小
        double width = CompileFlowGeometryConstants.BREAK_GEOMETRY[0];
        double height = CompileFlowGeometryConstants.BREAK_GEOMETRY[1];
        return new BaseGeometry(x, y, width, height);
    }

    @Override
    public BaseCell createCell(Geometry geometry) {
        return new CompileFlowBreakCell(getModel(), geometry);
    }
